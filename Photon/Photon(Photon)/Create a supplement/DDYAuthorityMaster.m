...
+ (void)showAlertController:(AuthorizedFinishBlock)block device:(NSString *)device {
    ...
    [((AppDelegate *)[UIApplication sharedApplication].delegate).window.rootViewController presentViewController:alertC animated:YES completion:block];
}
...
+ (void)showAlertWithAppName:(NSString *)appName device:(NSString *)device
{
    ...
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:DDYLocalStr(@"PermissionDenied") message:LC_NSSTRING_FORMAT(@"%@%@", DDYLocalStr(@"PermissionTurnOn"), device) preferredStyle:UIAlertControllerStyleAlert];
    [alertC addAction:[UIAlertAction actionWithTitle:CancelText style:UIAlertActionStyleCancel handler:nil]];
    [alertC addAction:[UIAlertAction actionWithTitle:OKText style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];
    }]];
    [((AppDelegate *)[UIApplication sharedApplication].delegate).window.rootViewController presentViewController:alertC animated:YES completion:nil];
}
            ...
            [[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL granted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (granted) {
                        ...
                        if (show) { [self showAlertWithAppName:@"SmartMesh App" device:DDYLocalStr(@"microphone")]; }
                        if (fail) { fail(); }
                    }
                });
            }];
            ...
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (granted) {
                        ...
                        if (show) { [self showAlertWithAppName:@"SmartMesh App" device:DDYLocalStr(@"camera")]; }
                        if (fail) { fail(); }
                    }
                });
            }];
            ...
            [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (status == PHAuthorizationStatusAuthorized) {
                        ...
                        if (show) { [self showAlertWithAppName:@"SmartMesh App" device:DDYLocalStr(@"album")]; }
                        if (fail) { fail(); }
                    }
                });
            }];
            ...
            ABAddressBookRequestAccessWithCompletion(addressBook, ^(bool granted, CFErrorRef error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (granted) {
                        ...
                        if (show) { [self showAlertWithAppName:@"SmartMesh App" device:DDYLocalStr(@"address book")]; }
                        if (fail) { fail(); }
                    }
                });
            });
...
+ (void)locationAuthSuccess:(AuthorityBlock)success fail:(AuthorityBlock)fail alertShow:(BOOL)show
{
    ...
    switch (CLstatus)
    {
        ...
        case kCLAuthorizationStatusDenied:
        {
            ...
            if (show)
            {
                [self showAlertWithAppName:@"SmartMesh App" device:DDYLocalStr(@"location")];
            }
            if (fail)
            {
                fail();
