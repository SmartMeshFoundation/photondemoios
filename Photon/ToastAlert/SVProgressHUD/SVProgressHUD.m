...
//    [[self sharedView] showImage:image status:status duration:1.5];
    [[self sharedView] showImage:image status:status duration:MIN(displayInterval, 1)];
}
...
+ (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MAX((float)string.length * 0.06 + 1, [self sharedView].minimumDismissTimeInterval);
}
