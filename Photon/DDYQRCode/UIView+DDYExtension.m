...
//  Copyright © 2015 634778311 All rights reserved.
//
...
#pragma mark 获取相对屏幕位置
- (CGRect)relationToWindow
{
    UIWindow *keyWindow = [[[UIApplication sharedApplication] delegate] window];
    if (!keyWindow) keyWindow = [[UIApplication sharedApplication] keyWindow];
    return [self convertRect:self.bounds toView:keyWindow];
}
...
#pragma mark 添加单侧边框线
- (void)ddy_BorderTop:(BOOL)top left:(BOOL)left bottom:(BOOL)bottom right:(BOOL)right color:(UIColor *)color width:(CGFloat)width
{
    if (top) {
